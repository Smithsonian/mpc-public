...
```bash
pip install mpc-client
```

For DataFrame support:

```bash
pip install mpc-client[dataframe]
```
...
A jupyter notebook tutorial covering the basics of using `mpc_client` is available
...
The `.github` workflow should release a new version of `mpc_client` to `TestPyPI` every time
a PR is opened or updated.
If reviews would like to test the latest version of `mpc_client` from TestPyPI, you can run:
```bash
pip install --index-url https://test.pypi.org/simple/ --extra-index-url https://pypi.org/simple/ mpc-client
```
where `--index-url` points to TestPyPI for mpc-client itself, while `--extra-index-url` falls back to real PyPI for its
dependencies (requests, pandas, etc.) which aren't on TestPyPI.


### Release

A `.github` workflow will release a new version of `mpc_client` to `PyPI`
every time a PR that touches `mpc_client` is merged into `main`,
**as long as the version is updated in `pyproject.toml`**.


### To-Do

There are a number of MPC APIs that are not yet implemented in `mpc_client`.
...
2. Have a corresponding method added to `mpc_client` that is tested and documented.
