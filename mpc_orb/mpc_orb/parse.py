...
class MPCORB:
    ...
    def describe(self, variable_name, version=None):
        ...
        if variable_name in self.schema_json["properties"].keys():
            return {
                variable_name: {
                    "description": self.schema_json["properties"][variable_name][
                        "description"
                    ],
                    "allowed properties": list(
                        self.schema_json["properties"][variable_name][
                            "properties"
                        ].keys()
                    ),
                }
        ...
        # (iii) If variable_name is an attribute of COM
            ...
            def_var_name = self.schema_json["properties"]["COM"]["properties"][
        ...
        return (
            {variable_name: None}
            if def_var_name is None
            else {variable_name: self._clean(self.schema_json["$defs"][def_var_name])}
        )
    ...
    def _generate_square_CoV(
        ...
        for i in range(num_params):
            for j in range(i, num_params):
                self.__dict__["covariance_array"][i, j] = self.__dict__[
                    "covariance_array"
                ][j, i] = self.__dict__["covariance"]["cov%d%d" % (i, j)]

    def _generate_element_dict(
        self,
    ):
