...
**Tip:** The [`mpc_client`](https://github.com/Smithsonian/mpc-public/tree/main/mpc-client) Python package (`pip install mpc-client`) wraps all of these APIs into a single `MPCClient` class. Each tutorial below shows both the raw `requests` approach and the equivalent `mpc_client` usage.

<div id="contents-grid"></div>

 - [**Getting Started with `mpc_client`**](notebooks/mpc_tutorial_mpcapi.ipynb)
