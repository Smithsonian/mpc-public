...
Returns:
   true if in model, bin indexes returned in parameter 'bin' in order q, e, i, h.
   false if out of model
*/
_Bool qeihToBin(double q, double e, double i, double h, int bin[4]) {
   return qeiToBin(q, e, i, bin) && hToBin(h, bin+3);
}
...
// any of: q < 1.3, e > .5, or i > 40
_Bool isMpcint(double q, double e, double i) {
   return q < 1.3 || e >= .5 || i >= 40.;
}

// 'NEO' objects
// The definition of NEO implemented here is q < 1.3
_Bool isNeo(double q, double e, double i) {
   return q < 1.3;
}

// Mars Crosser
// 1.3 <= q < 1.67, Q > 1.58
_Bool isMarsCrosser(double q, double e, double i) {
...
_Bool isHungaria(double q, double e, double i) {
   if (e > .18 || i < 16 || i > 34)
      return 0;
   double a = q/(1-e);
   return a < 2 && a > 1.78;
...
_Bool isPhocaea(double q, double e, double i) {
   if (q < 1.5 || i < 20 || i > 27)
      return 0;
   double a = q/(1-e);
   return a < 2.45 && a > 2.2;
...
_Bool isInnerMB(double q, double e, double i) {
   if (q < 1.67)
      return 0;
   double a = q/(1-e);
...
_Bool isHansa(double q, double e, double i) {
   if (e > .25 || i < 20 || i > 23.5)
      return 0;
   double a = q/(1-e);
...
_Bool isPallas(double q, double e, double i) {
   if (e > .35 || i < 24 || i > 37)
      return 0;
   double a = q/(1-e);
   return a < 2.8 && a > 2.5;
...
// Mid Main Belt
// 2.5<a<2.8, e<.45, i<20
_Bool isMidMB(double q, double e, double i) {
   if (e > .45 || i > 20)
      return 0;
   double a = q/(1-e);
   return a < 2.8 && a > 2.5;
...
_Bool isOuterMB(double q, double e, double i) {
   if (e > .4)
      return 0;
   double a = q/(1-e);
...
// Hildas
// 3.9<a<4.02, e<.4, i<18
_Bool isHilda(double q, double e, double i) {
   if (i > 18 || e > .4)
      return 0;
   double a = q/(1-e);
   return a > 3.9 && a < 4.02;
...
// 2 < T < 3, q >= 1.3
_Bool isJFC(double q, double e, double i) {
   if (q < 1.3)
      return 0;
   double t = 5.2*(1-e)/q + 2*sqrt(q*(1+e)/5.2)*cos(i*M_PI/180.);
   return t < 3 && t > 2;
}
