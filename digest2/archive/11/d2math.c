...
/* d2math.c
 * 
 * Copyright (c) 2010 Sonia Keys Copyright (c) 2005,2006 Kyle Smalley,
...
void setupVectors(tracklet * tk, double d)
{
    tk->dAnyTag = 0;
    ...
    for (int i = 0; i < 2; i++, obsp++) {
        ...
        double dec = obsp->dec + (tkRand(tk) * 2 - 1) * tk->obsErr[i];
        double cosdec = cos(dec);
        double ra =
            obsp->ra + (tkRand(tk) * 2 - 1) * tk->obsErr[i] * cosdec;
        v[0] = cos(ra) * cosdec;
        v[1] = sin(ra) * cosdec;
        v[2] = sin(dec);
...
void score(tracklet * tk)
{
    ...
    for (int c = 0; c < nClassCompute; c++, cl++) {
        ...
        cl->rawScore = d > 0. ? 100. * cl->sumAllInClass / d : 100.;

        d = cl->sumUnkInClass + cl->sumUnkOutOfClass;
        cl->noIdScore = d > 0. ? 100. * cl->sumUnkInClass / d : 100.;
    }
}
