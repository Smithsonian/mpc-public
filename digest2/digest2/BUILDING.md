...
The C CLI does not build natively on Windows (it requires pthreads and
POSIX APIs). Windows users should use the Python package (`pip install digest2`),
which provides pre-built wheels with no external C library dependencies
